...
import (
	...
	dataactions "github.com/jsdrews/tuilib/examples/data/actions"
	...
	datamultiselect "github.com/jsdrews/tuilib/examples/data/multiselect"
	datapoll "github.com/jsdrews/tuilib/examples/data/poll"
	datapolltable "github.com/jsdrews/tuilib/examples/data/polltable"
	dataremote "github.com/jsdrews/tuilib/examples/data/remote"
	...
	datatreeactions "github.com/jsdrews/tuilib/examples/data/treeactions"
...
var entries = []entry{
	...
	{"Tree — searchable expand/collapse", "A synthetic project tree with cursor, expand/collapse (←→/space), search (/), and filter mode (\\) that hides non-matching subtrees. x marks a node (space stays expand/collapse), X or shift+click marks a range, A marks every visible row, D drops — the ✓ gutter is leftmost so it lines up at every depth, and marking a branch marks that node alone. Leaves carry colored status icons (lipgloss-rendered) so the row highlight stays intact across ANSI segments.", datatree.New},
	...
	{"Remote — paged source behind a sparse table", "The whole windowed-source loop: pkg/source coordinating a pkg/table in FilterRemote/SortRemote over a simulated 5,000-row API that answers one 100-row page at a time with 250ms of latency. Scroll faster than it answers and you see the \"·\" placeholders the table draws for rows it hasn't received; the cursor stays put and the data arrives under it. / filters at the source (enter commits — one request, not one per keystroke), [/]/s sorts there too, r refetches the current window. Tab-completion candidates come from SetDistinct, since one page can't know every region.", dataremote.New},
	...
	{"Actions — the verb menu (pkg/action)", "Press a or right-click a row to open action.Menu. It sizes itself to its widest row and anchors where you asked. This screen is single-target; Multi-select is the demo that marks rows. Start a Restart and reopen the menu to see the Exclusive gate. Single click commits, click-away dismisses, Delete confirms first.", dataactions.New},
	{"Multi-select — mark rows, act on the set", "table.Model with Options.Markable: space marks the cursor row, V (or shift+click) extends the selection between the last-marked row and the cursor in either direction, A marks everything the filter shows, D drops the selection, and clicking the ✓ gutter toggles without opening the row. Marks are held by Key (SetKeyedRows), so they survive filtering — mark a row, filter it away, and the count on the border does not move — and survive a theme swap, because SetTheme carries them with SetMarks. Press a or right-click for the menu: it is titled with what it will act on (\"3 items\"), and Describe, which did not declare Multi, dims itself with a reason as soon as a second row is marked.", datamultiselect.New},
	{"Tree actions — mark nodes, act on the subtree", "pkg/tree marking + pkg/action on a cluster-shaped hierarchy. x marks a node at any depth, X a range, A everything visible, D drops. Marking a branch marks that branch alone — the screen resolves it to the pods a verb should touch with a prefix test on the path, which is why Restart cascades into a marked namespace and Describe does not. Marks survive collapsing, so the menu title counts what it will actually touch (\"Restart 9 pods\") even when some are folded out of sight. Delete is guarded to staging.", datatreeactions.New},
